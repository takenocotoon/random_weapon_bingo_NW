...
export function openSubMenu(element:Element) {
    if (!element) return;
    if (element.classList.toggle('is_active')) {
        const submenus = document.getElementsByClassName('js-submenu');
        Array.from(submenus).forEach(target => {
            if (target !== element && target.classList.contains('is_active')) target.classList.remove('is_active');
        });
    }
}
...
export function openDialog(id:string) {
    const target: HTMLDialogElement = document.getElementById(id) as HTMLDialogElement;
